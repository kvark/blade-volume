...
- Exact-blend ranking finds opposing spatial camera evidence in 3,691 of 3,793
  ...
  paths and propagate disparity before optical support is assigned.
...
| Controlled-light diversity | LUCES-MV and DiLiGenT-MV selected; OLATverse two-axis light transfer passes; one aligned construction OLAT improves held-camera means on five objects but has small tail/coverage tradeoffs; physical Gaussian, generic densification, radial support splitting, additive/partitioned/pre-fusion 104-light response tracks, robust-albedo density training, conditional absorption depth, post-depth normal integration, direct expected-depth photometric-normal loss, direct calibrated multi-light diffuse density fitting (joint and material-warmed), a scratch 104-way response-table density fit, global/local/post-extraction sparse-track propagation, standalone response-track clouds, hard/soft/local/appended response-track initialization at 16K and 64K, broad and coherent response-track patch composition, global/coarse/fine response continuations, visibility-only material coupling, a bounded finite-light bounce, and projected/exact-blend normal attribution are rejected; dense 27-light signature matching is selected as upstream correspondence evidence at 99.32%/96.18% within-one-pixel accuracy at width 64/128, but its standalone and fallback render paths fail on coverage and 185/192 tails; post-formation track geometry is closed; C452 point truth and excluded C769 polarized normals localize the first error before fusion; construction OLATs recover a 24.66-degree photometric-normal signal; residual decomposition finds local point/light response rather than global calibration or a half-vector lobe; direct GGX selected and finite visibility remains opt-in | Propagate dense disparity from verified 27-light epipolar matches while camera rays and source observations still own samples; require cross-view and mask-hull consistency before assigning point support, and keep light-specific appearance out of the asset | Improve every C452 construction aggregate and tail without adding polygonal geometry, then improve C769 construction and its excluded polarized-normal metric before preserving the result on untouched C713/C777 |
...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected | Attribute the two OLAT tail losses and the Bear regression on construction-only data before changing transport again | Both excluded-light mean/tail improve with no coverage regression or GPU fault |
