...
- Finite point lights now evaluate the existing roughness/F0 fields with direct
  GGX, sharing a CPU reference and one runtime shader path. A first C276
...
- One finite-distance visibility ray reuses that shader and its existing ray
  ...
  backend-mixed and DiLiGenT-MV Bear regresses. An exact visibility-aware
  diffuse equation lowers its own linear loss but regresses C452 construction
  means by 0.083/0.059 dB and one image by 0.414/0.436 dB whole/foreground;
  its CPU BVH prototype is removed. Visibility without the captured indirect
  ...
  is removed too. Exact-blend gradients leave only 418/198/1 of 6,840 surfels
  consistent by light/camera/image and still miss one tiny production tail.
  Split contradictory cloud support before asking one normal to explain it.
...
- Exact-blend ranking finds opposing spatial camera evidence in 3,691 of 3,793
  ...
  radial kernels, so the host prototype is removed. Full 104-light matching
  then establishes 25 coherent, validation-safe sites. Twenty-three already
  lie inside an existing radial support while their normals differ by 52
  ...
  Applying response compatibility before fusion either removes 242 particles
  and 0.09 recall points or, with exact fallback, lowers every aggregate and
  ...
  not another proxy, fusion threshold, or segment statistic. A direct
  construction-only normal loss on differentiable expected absorption depth is
  now rejected too: density-only regresses 106/192 images, while coupled site
  motion drops traced-hit coverage from 99.1% to 97.0%. A per-view derivative
  does not anchor absolute layer or optical ownership. A direct follow-up fits
  ...
  graph/API/CLI path is removed. A scratch 104-way per-cell response shared
  across cameras tests the remaining direct identity hypothesis. It ties raw
  traced support and is far safer than direct diffuse fitting, but still loses
  `0.0026` dB whole mean, 0.0147 recall points, 106/192 foreground tails, and
  0.317 dB on the worst pair. Its table and graph path are removed. Close
  radiance-driven density on this split and establish absolute cross-view
  first-surface correspondence upstream; visibility alone is not the missing
  switch. Four-light triangulation now supplies that cue sparsely: 579 C452
  tracks pass a disjoint-camera silhouette gate at 97.6%, and a later truth
  audit puts 94.0% within one source pixel. A global sparse-depth density
  continuation improves truth depth slightly but loses `0.127/0.038` dB
  construction whole/foreground mean, 0.455 precision points, and 102/192
  foreground tails, so its temporary loss is removed. The next step is a
  bounded, local, support-conserving point-cloud correction around those
  independently measured anchors. That direct follow-up is now closed too:
  denser matching produces 1,332/3,831 selected tracks at 128/256 pixels, but
  isolated or smooth center corrections improve some aggregates while losing
  precision and 58--88 image tails; normal replacement loses both means; and
  a track-only cloud is precise but cannot cover textureless regions. Seeding
  all 1,297 width-128 tracks before ordinary training improves established
  construction whole/foreground mean by `0.160/0.147` dB and recall by 0.656
  points, but loses 0.273 precision points and 69/192 tails. Fixing the exact
  same sites is decisively worse: traced support falls to 96.7%, precision by
  7.41 points, and 153/192 tails regress. Soft expected-depth association
  collapses tracing to 91.9%. Replacing nearest sites instead improves every
  aggregate (`+0.471/+0.310` dB whole/foreground and `+0.656/+0.609` points
  recall/precision), but still loses 52/192 tails; a 523-track topology-local
  subset and a 17,681-site append are worse. The predeclared 64K/256 scale
  check reduces median site displacement to 0.0265 world units, but foam
  whole/foreground PSNR falls `0.416/0.447` dB and surface precision loses 1.0
  point. After identical material fitting, foreground mean gains 0.212 dB but
  precision loses 0.459 point and 60/192 tails regress, including three by
  more than 1 dB. Every light mean is positive while six camera means are
  negative: isolated replacement changes view-dependent cell ownership even
  at production scale. All scratch paths are removed, the complete
  initialization family is closed, and C713/C777 remain untouched transfer
  gates.
  Continuous patch composition is closed too. Robust eight-neighbor track
  discs replace 11,959 fallback surfels and gain precision, but lose 0.403 dB
  foreground mean and 152/192 tails. The existing shared-view component rule
  plus point-only midpoint resampling produces 8,103 samples and improves the
  merged surface's median released-truth distance and normal error, yet loses
  0.317 dB foreground mean and the same 152 tails. Better sparse geometry still
  assigns radial support and view ownership incorrectly after surface
  formation; no patch code or threshold is retained.
  A 27-light per-pixel response descriptor then establishes a better upstream
  seed: 99.32%/96.18% of width-64/128 matches lie within one source pixel of
  C452 truth. Keeping all already-triangulated width-64 candidates yields 3,194
  oriented tracks and 7,351 coherent point samples at unchanged accuracy, but
  only 51.66% mask recall. A mask-hull fallback loses 0.836 dB foreground and
  185/192 tails. Select the correspondence signal only; reject both rendering
  paths. Dense camera-ray disparity propagation now produces 9,683 C452
  surfels and improves the official held-light/held-camera whole/foreground
  ...
  recall-restored candidate loses 90/192 image tails. Layer-aware fusion keeps
  9,698 surfels but loses foreground mean, recall, and 98/192 tails.
  Neighbor- and mask-derived anisotropic Gaussian controls are also negative.
  Keep the disparity signal and close support-only initialization changes.
  Existing visibility cuts the recall-restored candidate to 52/192 tails and
  makes seven of eight light means positive, localizing the next dependency to
  light transport rather than another cloud topology.
...
| Controlled-light diversity | LUCES-MV and DiLiGenT-MV selected; OLATverse two-axis light transfer passes; dense 27-light signatures and camera-ray disparity are selected as upstream correspondence evidence; the C452 dense candidate improves all official held-light and held-camera means, but loses recall and 48/618 tails; finer/layer-aware fusion, anisotropy, post-formation geometry, generic densification, direct density, finite-light bounce, point-owned response, direct photometric-normal transfer, separate shading normals, photometric pre-fusion, scene-wide wrapped diffuse, and complete-render visibility-aware normal-only fitting all fail strict C769 tails; an exact-blend degree-1 oracle passes C769 but loses five C452 tails, and spatial degree-2 reduces that to two rather than zero; complete-render and multi-light continuation guards now reject every individual image regression; direct GGX is selected and finite visibility remains opt-in | Couple point centers, opacity/support, normals, and physical transport in one bounded proposal under the selected image-tail guard | Improve every C769 and C452 construction-light validation mean and tail before opening C713 once as a fresh gate; keep C777 untouched |
...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected; on dense C769 visibility cuts construction regressions from 90 to 52; exact compositing proves that light-direction response can gain 1.60 dB on C769, but scalar/RGB, degree-1/2, spatially regularized, no-cosine PRT, sRGB-weighted PRT, direct GGX refit, continuous light-by-view, fold-consistent photometric-normal, scene-wide wrapped-diffuse, and complete-render visibility-aware normal-only variants all miss image tails; the exact CPU oracle agrees with production GPU scores within 0.0069 dB; complete-render point refinement and multi-light Gaussian continuation now both restore on any individual image regression | Replace normal-only perturbations with a coupled support/normal/transport proposal; keep full compositing and the balanced tail guard inside acceptance | Every C769/C452 internal mean and tail and then the fresh C713 transfer gate improve with fixed coverage semantics, no light-ID lookup, and no GPU fault |
| Radiometry and transport | attributed; scalar correction and extra samples rejected; C769 bad-camera gains vary 4.2x by light and light-099 spatial correlation is only 0.60; a jointly fitted global gain still loses 12/64 disjoint pairs and the physical wrap fit selects zero | Preserve measured light scale; treat the repeated `(light, view)` residual as surface/response evidence, not exposure | A correction repeats across objects and every known-light tail before either excluded light is opened |
...
- [x] Keep exact Gaussian geometry, alpha, grouping, and depth order, but shade
  each surface-depth group with only its maximum-response contributor rather
  than its coverage-weighted physical mixture. About 74% of Bear groups have
