...
| Controlled-light diversity | LUCES-MV and DiLiGenT-MV selected; OLATverse two-axis light transfer passes; dense 27-light signatures and camera-ray disparity are selected as upstream correspondence evidence; the C452 dense candidate improves all official held-light and held-camera means, but loses recall and 48/618 tails; finer/layer-aware fusion, anisotropy, post-formation geometry, generic densification, direct density, finite-light bounce, point-owned response, direct photometric-normal transfer, separate shading normals, photometric pre-fusion, and scene-wide wrapped diffuse all fail strict C769 tails; an exact-blend degree-1 oracle passes C769 but loses five C452 tails, and spatial degree-2 reduces that to two rather than zero; the multi-light continuation guard now measures every fitted light-camera pair and rejects any individual regression; direct GGX is selected and finite visibility remains opt-in | Put the same balanced image semantics inside the oriented-point visibility/indirect objective, rather than adding another frozen response field | Improve every C769 and C452 construction-light validation mean and tail before opening C713 once as a fresh gate; keep C777 untouched |
