...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected; on dense C769 visibility cuts construction regressions from 90 to 52 and makes 7/8 light means positive; a center-pixel degree-2 light-direction oracle loses both fitted- and disjoint-light means | Refit the same smooth directional basis through exact surface-blend equations; add no runtime field until it transfers across construction lights and C452 | Both excluded-light mean/tail improve with no coverage regression, light-ID lookup, or GPU fault |
