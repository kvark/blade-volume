...
| Controlled-light diversity | LUCES-MV and DiLiGenT-MV selected; OLATverse two-axis light transfer passes; dense 27-light signatures and camera-ray disparity are selected as upstream correspondence evidence; the C452 dense candidate improves all official held-light and held-camera means, but loses recall and 48/618 tails; finer and layer-aware fusion plus neighbor/mask anisotropy all fail C769 construction; post-formation track geometry, generic densification, direct radiance-driven density, isotropic support inflation, visibility-only coupling, bounded finite-light bounce, and a one-owner degree-2 directional residual are rejected; an exact-blend degree-1 oracle improves every C769 disjoint-light image but loses five C452 tails after stronger regularization; direct GGX is selected and finite visibility remains opt-in | Keep exact compositing but spatially regularize low-order response across compatible points; validate without light-index lookup or geometry changes | Improve every C769 and C452 construction-light validation mean and tail, then open C713 once as a fresh transfer gate while C777 remains untouched |
...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected; on dense C769 visibility cuts construction regressions from 90 to 52 and makes 7/8 light means positive; a center-pixel degree-2 oracle fails, while an exact-blend degree-1 oracle gains 1.60 dB on C769 disjoint-light foreground but remains tail-mixed on C452; achromatic response and visibility controls do not fix it | Add spatial/normal compatibility to the offline exact-blend solve; add no runtime field until it transfers to fresh C713 | Every C769/C452 internal mean and tail and then the fresh C713 transfer gate improve with no coverage regression, light-ID lookup, or GPU fault |
