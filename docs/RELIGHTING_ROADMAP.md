...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected; on dense C769 visibility cuts construction regressions from 90 to 52; exact compositing proves that light-direction response can gain 1.60 dB on C769, but scalar/RGB, degree-1/2, spatially regularized, no-cosine PRT, sRGB-weighted PRT, direct GGX refit, and continuous light-by-view variants all fail C452 tails; the exact CPU oracle agrees with production GPU scores within 0.0069 dB | Close generic frozen-surface response growth; fit visibility/indirect transport jointly with normals and support, using a complete-render objective that cannot trade one image for another | Every C769/C452 internal mean and tail and then the fresh C713 transfer gate improve with fixed coverage semantics, no light-ID lookup, and no GPU fault |
