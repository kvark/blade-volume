...
| Light transport | sampled environment visibility/bounce and direct finite-light GGX selected; finite-light visibility implemented but opt-in; bounded finite-light bounce rejected; on dense C769 visibility cuts construction regressions from 90 to 52 and makes 7/8 light means positive | Test a smooth point-local light-direction basis as an offline oracle; add no runtime field until it transfers across construction lights and C452 | Both excluded-light mean/tail improve with no coverage regression, light-ID lookup, or GPU fault |
